...
func (y *Yarn) Build(buildDir, cacheDir string) error {
	...
	installArgs := []string{"install", "--production", "--pure-lockfile", "--ignore-engines", "--cache-folder", filepath.Join(cacheDir, ".cache/yarn"), "--modules-folder", filepath.Join(buildDir, "node_modules")}
